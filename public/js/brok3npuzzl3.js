...
var hash = '27yqDOg+5wfvjiJ0HRSbPpA4alM69NtrCVnZdWQBXhok_zYu8GcesIxF31EUTLKm';
...
var rowsUnscrambleLevel = 2;
var columnsUnscrambleLevel = 2;
...
function drawImage(matrix, ctx) {
    ...
    for (var i = 0; i < image.data.length; i = i + 4) {
        ...
        image.data[i + 0] = matrix[row][column][0];
        image.data[i + 1] = matrix[row][column][1];
        image.data[i + 2] = matrix[row][column][2];
        image.data[i + 3] = matrix[row][column][3];
    }
...
function finalUnscramble(matrix, order) {
    ...
    for (var i = 0; i < base.length; i++) {
        ...
        for (var j = 0; j < auxMatrix.length / base.length; j++) {
            ...
            auxMatrix[order[i] * auxMatrix.length / base.length + j] = matrix[pos];
            pos++;
        }
    }
...
// We initialize the program here
decodeImage(base64Image, "lettersImg", function() {
    decodeImage(window.artworkImg, "artworkImg", app.init, app);
}, window);
